...
class EssayContextGenerator(BaseContextGenerator):
    def __call__(self, model: str, context_tokens: int, max_tokens: int = None) -> ([dict], int):
        """
...
class StoryContextGenerator(BaseContextGenerator):
    def __call__(self, model: str, context_tokens: int, max_tokens: int = None) -> ([dict], int):
        """
        Generate `messages` array based on tokens and max_tokens.
        Output includes an excerpt of information from the wikipedia page for
        machine learning, along with a request for an essay of 3*max_tokens.
        
        NOTE: This method tends to result in ~1000-1300 token responses for both
                ...
                messages.append(
                    {
                        "role": "user",
                        "content": (
                            f"{str(time.time())} I am in need of a long story that will hold my attention and relax me."
                            " Please create a long, winding story of at least {max_tokens*3} words."
                        )
                    }
                )
            messages_tokens = 0
...
# class MLBlogPostContextGenerator(BaseContextGenerator):
#     def __call__(self, model: str, context_tokens: int, max_tokens: int = None) -> ([dict], int):
#         """
#         Generate `messages` array based on tokens and max_tokens.
#         Output includes an excerpt of information from the wikipedia page for
...
#         NOTE: This method tends to result in ~700-1000 token responses.
...
#             "role": "user",
#             "content": (
#                 f"{str(time.time())} You are a blog post content writer. "
#                 "Please create a long-form blog post that educates readers on the different types of machine learning models, explaining how each model works within 1-2 paragraphs. Use any prior context to help write the article. "
#                 f"The blog post should be at least {max_tokens*3} words. DO NOT stop before you have finished writing the blog post, and make sure to include a conclusion that brings together the central themes in the post."
#             )
#         }]
#         current_tokens = num_tokens_from_messages(messages, model)
#         if len(CACHED_PROMPT) > 0:
#             messages.insert(0, {"role": "user", "content": str(time.time()) + "\nContent:\n"})
#             messages[0]["content"] += " " + CACHED_PROMPT
#         elif (context_tokens is not None) and (current_tokens + 30 < context_tokens):
#             messages.insert(0, {"role": "user", "content": str(time.time()) + "\nContent:\n"})
#             current_tokens = num_tokens_from_messages(messages, model)
#             ml_wiki_context_idx = 0
#             while current_tokens < context_tokens:
#                 current_tokens = num_tokens_from_messages(messages, model)
#                 num_additional_words = math.ceil((context_tokens - current_tokens) / 4)
#                 messages[0]["content"] += " " + " ".join(
#                     WIKIPEDIA_MACHINE_LEARNING_PAGE_TEXT.split()[ml_wiki_context_idx: ml_wiki_context_idx+num_additional_words]
#                 )
#                 ml_wiki_context_idx += num_additional_words
#                 current_tokens = num_tokens_from_messages(messages, model)        
#             CACHED_PROMPT = " ".join(WIKIPEDIA_MACHINE_LEARNING_PAGE_TEXT.split()[:ml_wiki_context_idx+num_additional_words])
#             CACHED_MESSAGES_TOKENS = current_tokens
...
class AutoContextGenerator(BaseContextGenerator):
    ...
    def __init__(self):
        ...
        self._model_mapping = {
            "gpt-4": "story_generator",
            "gpt-4-0314": "story_generator",
            "gpt-4-32k-0314": "story_generator",
            "gpt-4-0613": "story_generator",
            "gpt-4-32k-0613": "story_generator",
            "gpt-3.5-turbo": "story_generator",
            "gpt-3.5-turbo-0613": "story_generator",
            "gpt-3.5-turbo-16k-0613": "story_generator",
        }
...
GENERATOR_NAME_TO_CLASS = {
    "essay_generator": EssayContextGenerator,
    "story_generator": StoryContextGenerator,
    "auto": AutoContextGenerator,
}

# WIKIPEDIA_MACHINE_LEARNING_PAGE_TEXT = """
